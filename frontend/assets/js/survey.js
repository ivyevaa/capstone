document.addEventListener("DOMContentLoaded", () => {
  ...
  ["step2Questions", "step3Questions", "step4Questions"].forEach((host, hostIndex) => {
    ...
    node.innerHTML = questionText.slice(hostIndex * 5, hostIndex * 5 + 5).map((text, index) => {
      ...
      return `<div class="question-block mb-4"><p class="fw-semibold">${number}. ${text}</p><div class="likert-grid" role="radiogroup" aria-label="Question ${number}">${[5, 4, 3, 2, 1].map((value) => `<input id="q${number}_${value}" name="q${number}" value="${value}" type="radio"><label for="q${number}_${value}">${value}</label>`).join("")}</div></div>`;
    }).join("");
  });
  ...
  if (consentCheck && startButton) {
    ...
    startButton.addEventListener("click", () => {
      localStorage.setItem("ttConsent", "accepted");
  ...
  if (consentGate && localStorage.getItem("ttConsent") !== "accepted") {
    bootstrap.Modal.getOrCreateInstance(consentGate, { backdrop: "static", keyboard: false }).show();
  }
  ...
  surveyForm?.addEventListener("change", () => {
    TraitTracker.qs("#autoSaveStatus").innerHTML = '<i class="bi bi-cloud-check"></i> Saved just now';
    const data = {};
  ...
  surveyForm?.addEventListener("submit", (event) => {
    ...
    TraitTracker.saveUser({ scores, lastAssessment: new Date().toISOString() });
